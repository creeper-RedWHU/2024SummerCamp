...
forecast::forecast(QWidget *parent)
    : QWidget(parent), progressDialog(new QProgressDialog("正在预测，请稍候...", "取消", 0, 0, this))
{
    database thisdb;
    ...
    QStackedWidget *picwidget = new QStackedWidget;

    // 创建左上角的文本标签
    QLabel *title = new QLabel("基于机器学习的气温趋势预测", picwidget);
    ...
    mainLayout->addWidget(picwidget);
...
void forecast::onPredictClicked()
{
    ...
    connect(forecast_api, &api::imageReady, this, &forecast::onPredictionReady);

    QPixmap result;
    forecast_api->ml_sarima(cityBox->currentText(), result);
}

void forecast::onPredictionReady(const QPixmap &pixmap)
{
    progressDialog->close(); // 关闭进度对话框
    draw(pixmap);
}

void forecast::draw(const QPixmap &pixmap)
{

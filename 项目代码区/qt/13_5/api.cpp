...
bool api::ml_sarima(const QString& city,QPixmap& bpicture)
{
    // 检查networkManager是否已经初始化
    if (!networkManager) {
        qDebug() << "networkManager未初始化";
...
// 处理SARIMA API的回复，发射imageReady信号
void api::handleml_sarimaReply()
{
    ...
    if (reply->error() == QNetworkReply::NoError)
    {
        ...
        // 读取响应数据
        QByteArray response_data = reply->readAll();
        qDebug() << "响应数据大小:" << response_data.size();

        QPixmap pixmap;
        if (pixmap.loadFromData(response_data)) {
            qDebug() << "图片数据成功加载到QPixmap";
            emit imageReady(pixmap);
        } else {
            qDebug() << "图片数据加载到QPixmap失败";
        }
    }
    else {
        QString errorString = reply->errorString();

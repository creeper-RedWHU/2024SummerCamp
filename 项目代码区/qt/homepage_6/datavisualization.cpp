...
int cityidx=0, yearidx=0, monthidx=0,stardayidx=0,enddayidx=0;
    ...
    , ui(new Ui::datavisualization)
{
    ...
    QObject::connect(YearComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
                     [=](int index) mutable {
                         yearidx = index;
                     });
    QObject::connect(MonthComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
                     [=](int index) mutable {
                         monthidx = index;
                     });
    QObject::connect(endDayComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
                     [=](int index) mutable {
                         enddayidx = index;
                     });
    QObject::connect(startDayComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
                     [=](int index) mutable {
                         stardayidx = index;
                     });
   ...
   QObject::connect(drawButton, &QPushButton::clicked, this, &datavisualization::mydraw);
    ...
    //根据数据绘制图表
        ...
        query.exec("SELECT day, max_temperature FROM climate where city =:city and year =:year  and month = :month and day<=:eday and day>=:sday");
        ...
        query1.exec("SELECT weather FROM climate  where city =:city and year =:year  and month = :month and day<=:eday and day>=:sday");
        int count1=0;  //阴
        int count2=0;  //小雨
        int count3=0;  //晴
        int count4=0;  //多云
        ...
        *barset8 << wind8;
        ...
        query3.exec("SELECT day, min_temperature FROM climate where city =:city and year =:year  and month = :month and day<=:sday and day>=:eday");
